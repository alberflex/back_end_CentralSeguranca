...
export class PorteiroResource implements IPorteiroRepository {
    public async cadastrarPorteiro(porteiro: Porteiro): Promise<Porteiro> {
        ...
        const cadastraPorteiro = await pool.request()
            ...
            `)
        return cadastraPorteiro.recordset[0] as Porteiro;
    }
    ...
    public async deletarPorteiro(id: number): Promise<Porteiro> {
        const pool = await conexaoMSSQL();
        const resultado = await pool.request().input("id", sql.Int, id).query(`DELETE FROM cs_porteiro WHERE id = @id`);

        return resultado.recordset?.[0] as Porteiro;
    }

    public async listarTodosPorteiros(): Promise<Porteiro[]> {
        const pool = await conexaoMSSQL();
        const resultado = await pool.request().query(`SELECT * FROM cs_porteiro WHERE papel = 'PORTEIRO'`)

        return resultado.recordset as Porteiro[];
    }

    public async listarPorteiroPorId(id: number): Promise<Porteiro> {
        const pool = await conexaoMSSQL();
        const resultado = await pool.request().input("id", sql.Int, id).query(`SELECT * FROM cs_porteiro where id = @id`);

        return resultado.recordset?.[0] as Porteiro;
    }

    public async buscarPorChapa(chapa: string): Promise<Porteiro> {
        const pool = await conexaoMSSQL();
        const result = await pool.request().input("chapa", sql.VarChar(150), chapa)
            .query(`SELECT id, cpf, chapa, nome, senha, papel FROM cs_porteiro WHERE chapa = @chapa`);

        return result.recordset?.[0] as Porteiro;
    }

    public async editarUsuario(id: number, senha: string | null, papel: string): Promise<Porteiro> {
        const pool = await conexaoMSSQL();
        ...
        return resultado.recordset?.[0] ?? Porteiro;
    }
}